...
final class SerialNumberTest extends TestCase {
    public function testGetNewSerialNumber() {
        ...
        $this->assertSame('12.123.2023.03.0035', $serial_number->getNewSerialNumber($data, $pdo_link));
    }

    public function testGetLastSerialNumber() {
        $serial_number = new SerialNumber();
        try {
            $serial_number->_pdo_link = new PDO('mysql:dbname=test;host=localhost', 'root', '');
        } catch (PDOException) {
            die(json_encode(['error' => 'database connection failed']));
        }
