...
class SerialNumber
{
    ...
    public function getNewSerialNumber(array $recieved_data, PDO $pdo_link): string
    {
        ...
        Messages::echoMessage("$depart.$stand.$ordinal");
        return "$depart.$stand.$ordinal";
    }

    /**
     * Возвращает массив с серийным номером вида [  'serial_number_id' => x,
     *                                              'operator' => x,
     *                                              'depart_num' => xx,
     *                                              'stand_num' => xxx,
     *                                              'serial_num' => xxxx.xx.xxxx,
     *                                              'date' => YYYY-MM-DD h:min:sec
     *                                                                              ].
     * В случае ошибки позвращает на клиент json encoded string вида {'error' : 'текст ошибки'}.
     *
    ...
    public function getLastSerialNumber(): string|array
    {
        ...
        try {
            ...
            Messages::dieWithError('last serial number number was not received');
        }
    ...
    /**
     * Осуществляет проверку даты последней записи серийного номера из БД.
     * Если в текущем месяце записей не было порядковый номер обнуляется (xxxx.xx.xxxx => xxxx.xx.0000).
     *
    ...
    private function sendNewSerialNumberToDatabase(string $ordinal_number): void
    {
        ...
        $this->pdo_link->beginTransaction();
        $pdo_statement = $this->pdo_link->prepare("INSERT INTO `serial_number`
         (`serial_number_id`, `operator`, `depart_num`, `stand_num`, `serial_num`, `date`)
        ...
        try {
            ...
            $this->pdo_link->rollBack();
            Messages::dieWithError('new serial number not recorded to database');
        }
    }
}
